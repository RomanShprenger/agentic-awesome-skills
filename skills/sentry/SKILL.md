...
description: Use when the user asks to inspect Sentry issues or events, summarize
  recent production errors, or pull basic Sentry health data via the Sentry CLI; perform
  read-only queries using the `sentry` comman…
...
1. Install the Sentry CLI: `curl https://cli.sentry.dev/install -fsS | bash` <!-- security-allowlist: curl-pipe-bash -->
