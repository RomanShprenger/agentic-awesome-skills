---
name: sdlc-worker
description: Plugin-aware worker that executes a single AgentFlow pipeline stage for
  an assigned task. Detects plugin vs standalone mode, uses SendMessage handoffs for
  progress reporting and stage completion, rea…
args: --slot T<N> (required — which worker slot this terminal represents, e.g., T2,
  T3, T4, T5)
