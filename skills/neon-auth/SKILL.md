---
name: neon-auth
description: Add authentication to a new app. Use for "add auth", "add login", Neon
  Auth (Managed Better Auth), identity routing, sign-up, sign-in, password reset,
  email OTP, magic links, organizations, phone OTP…
metadata:
  parent: neon
...
| --- | --- |
| No existing auth | Default to Managed Better Auth. [Managed setup](#managed-setup), then [references/managed-auth.md]. |
