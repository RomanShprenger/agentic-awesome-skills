---
name: dos-dispatch
description: 'Plan and ship the next batch on one lane: run `dos-next-up`, acquire
  a lease with `dos arbitrate`, gate empty work, dispatch the packet, and archive
  the run. Use when a single lane should move end to…'
...
> against a live DOS workspace (`dos 0.30.0`); copy-paste, then read the RUNG.
