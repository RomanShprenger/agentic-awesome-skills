...
description: Java COLA 架构指南 - 阿里巴巴 COLA 整洁分层架构框架。用于 Java/Spring Boot 项目的 DDD 分层架构设计、代码结构规范、应用架构重构。触发条件：COLA、Java
  DDD、Spring Boot 架构、分层架构、整洁架构、六边形架构、洋葱圈架构、代码分层、模块划分、Gateway 模式、Repository 模式、CQRS、架构重构、职责分离、Maven
  多模…
...
详细代码示例请参考：
- **DTO 和 Response**: [references/dto.md]
- **Service 和执行器**: [references/service.md]
- **Gateway 模式**: [references/gateway.md]
- **COLA 组件详解**: [references/components.md]
