...
description: Use when the user wants to map or remap their second brain, refresh the
  vault's queryable metadata index, extract structured frontmatter from typed notes
  (books, meetings, people, journals, goals), f…
trigger: /second-brain-mapping
argument-hint: '[--metadata-only | --insights-only | --dry-run | --sample [N] | --force
  | --type <name>]'
