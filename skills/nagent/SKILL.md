---
name: nagent
description: Launch a new bb worker thread with the right project, model, worktree,
  and task brief. Use for nagent, launching a bb session, or a standalone bb agent.
  Covers thread creation; use bb-cli for managin…
...
For Cursor/Grok or Codex Max naming traps, read [provider details]. Verify examples against the current catalog; do not guess IDs or substitute a different provider.
