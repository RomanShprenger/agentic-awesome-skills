---
name: release
description: Cut a versioned release of the DOS kernel — bump the version, draft release
  notes, commit, tag, push to master, and create a GitHub release. The tag push triggers
  the gated PyPI publish pipeline (pub…
disable-model-invocation: false
user-invocable: true
allowed-tools: Read, Edit, Grep, Glob, Bash, Write
argument-hint: '[summary of changes] [--scope <theme-token>... (recommended)] [--whole-tree]
  [--lint-prefix]'
output_root: none
...
version drift, release-notes nuance) live in [`release-runbook.md`
...
Load [`release-runbook.md` **only if**:
...
Full rules + anti-patterns in [`release-runbook.md`.
