---
name: youtube-search
description: 'Use when the user wants to find YouTube content on any topic: searching
  for videos or channels, finding creators who cover a subject, discovering tutorials,
  talks, or expert discussions, or looking u…'
version: 1.6.0
user-invocable: true
compatibility: Requires internet access to reach transcriptapi.com. No additional
  runtimes or dependencies needed.
...
?q=innovation&sort=views&duration=long&upload_date=month" \
  -H "Authorization: Bearer $TRANSCRIPT_API_KEY" \
  -H "User-Agent: YourAgent/1.0"
```
...
```bash
curl -s "https://transcriptapi.com/api/v2/youtube/search?q=react+tutorial&type=playlist" \
  -H "Authorization: Bearer $TRANSCRIPT_API_KEY" \
  -H "User-Agent: YourAgent/1.0"
```
