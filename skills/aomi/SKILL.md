---
name: aomi
description: Aomi CLI — natural-language to executable on-chain transactions via an
  agent. Send a prompt like 'swap 1 ETH for USDC' and the agent stages calldata, simulates
  as a batch, and returns a queued tx for…
license: MIT
...
The agent stages txs that target a different chain than the thread. Pass `--rpc-url` matching the *queued tx*'s chain, not the thread chain. See [examples/cross-chain-swap/README.md for CCTP Ethereum → Base.
