---
name: memory-manager
description: Agent记忆健康管理（Memory Manager）。用户说'记忆太乱''整理记忆''检查记忆''记忆清理'时使用。扫描 Hermes
  记忆文件（MEMORY.md/USER.md），统计 token 占用、找过期/冗余/超长条目、体检会话库（state.db 体积与索引占比）、并从会话历史挖掘用户纠正与长期要求（提示哪些还没进记忆）。Memory
  health check for AI ag…
version: 1.1.0
author: 涛哥
license: MIT
metadata:
