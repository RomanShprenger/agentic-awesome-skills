...
description: 'Reinforces an AI agent by turning Monte Carlo''s reinforcement loop
  diagnosis into code fixes.

  Reads the daily reinforcement loop report for an agent''s workflows, ranks the diagnosed
  issues, proposes…'
when_to_use: "Use when the user wants to act on an AI agent's diagnosed health problems\
  \ and land fixes:\n\"fix my agent\", \"reinforce my agent\", \"improve my agent's\
  \ health\", \"what should I fix in\n<agent>\", \"open a PR for my agent's top issue\"\
  . Expects the user to name the agent; if they\ndon't, it lists available agents\
  \ and asks. The flow is user-gated: it surfaces the reinforcement\nloop diagnosis\
  \ and asks which workflow to dig into and which issue to fix before writing any\
  \ code.\nDo NOT use for:\n- investigating a single agent alert or trace (eval drop,\
  \ latency spike, one trace id) —\n  use monte-carlo-troubleshoot-agent-traces\n\
