...
description: Plugin-aware stateless orchestrator sweep for the AgentFlow pipeline.
  Detects plugin vs standalone mode, manages worker teams via TeamCreate, dispatches
  tasks via SendMessage handoffs, reads Kanban s…
source_repo: urrhb/agentflow
...
1. Log the failure
2. Fall back to comment-based tag updates on the PM tool
3. Continue sweep — do not block on message delivery
