---
name: dos-goal-gate
description: Ground a keep-working goal in evidence the worker did not author by wiring
  `dos hook stop` to refuse false done claims. Use for one self-stopping agent or
  loop worker; use `dos-witness-claim` for fol…
