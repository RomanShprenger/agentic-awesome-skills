...
version: 1.0.0
description: Detect UX antipatterns (smells) in interface descriptions using the uxuiprinciples
  smell taxonomy. Returns structured findings with matched symptoms, severity, and
  step-by-step remediation recipes. A…
...
env:
  UXUI_API_KEY:
    description: API key from uxuiprinciples.com (pro tier unlocks full smell taxonomy
      with remediation recipes, time estimates, and refactor prompts)
    required: false
