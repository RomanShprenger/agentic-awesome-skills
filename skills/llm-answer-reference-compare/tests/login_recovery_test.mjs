...
assert.equal(recovery.schemaVersion, "fact-check-x/capture-recovery@2");
...
assert.ok(recovery.instructions.some((item) => item.includes("只允许提交一次")));
assert.ok(recovery.instructions.some((item) => item.includes("agent-browser")));
