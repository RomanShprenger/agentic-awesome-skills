---
name: audit-flow
description: Interactive system flow tracing across CODE, API, AUTH, DATA, NETWORK
  layers with SQLite persistence and Mermaid export. Use for security audits, compliance
  documentation, flow tracing, feature ideat…
license: MIT
compatibility: Requires Python 3.8+ (stdlib only, zero dependencies). Optional pyyaml
  for YAML export. Git for merge/diff driver features.
metadata:
  author: ArunJRK
  version: 1.0.0
...
| **Default** | **When uncertain of flow state → query DB before proceeding**                             | _(any unlisted rationalization)_     |
