...
description: Audit the developer experience of a product, SDK, docs site, or SKILL.md
  by dropping multiple Claude subagents at it with only a tiny task prompt and real
  tools (WebFetch, Bash, Write). Agents must d…
compatibility: Requires an agent harness with subagent (Agent tool), WebFetch, and
  Bash support, plus network access. No CLI install needed; audited products may require
