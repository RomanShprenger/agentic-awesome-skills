---
name: slides-to-pdf
description: Use when the user asks to convert, export, or save an HTML slide deck
  as a PDF ("save the slides as PDF", "export the deck to PDF", "make a PDF of the
  presentation"). Works on decks built by the Slid…
license: MIT
compatibility: Needs a headless Chrome or Chromium binary (Playwright cache or system
  install), Python 3, and pypdfium2 for PDF verification (already installed, or one
  pinned pip download into a throwaway venv). Works offline otherwise. macOS and Linux;
  Windows untested.
metadata:
  author: Gleb Lukicov
  version: 1.0.1
