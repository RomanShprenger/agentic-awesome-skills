...
description: Find Twitter/X influencers to promote a product or brand. Use when asked
  to find influencers, discover Twitter accounts for partnerships, identify creators
  in a niche, or build an influencer outreach…
source: orthogonal
...
Output a ranked markdown table with ALL qualifying influencers. Use **full plain-text URLs** — do NOT use markdown link syntax like `[text]` or `[@handle]` because these often don't render as clickable links in all contexts. Instead, write out the full URL directly.
...
- **Use full URLs, not markdown links** — Write `https://x.com/handle` and `https://linkedin.com/in/name` as plain text in the results table. Markdown link syntax like `[@handle]` or `[Profile]` often doesn't render as clickable links and makes the output harder to use
