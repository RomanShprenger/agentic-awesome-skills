---
name: graphify
description: 'Use when the user invokes /graphify or asks to build, update, or query
  a knowledge graph from a folder of files (code, docs, papers, notes, images). Triggers:
  ''map this codebase'', ''graph my notes'', ''…'
trigger: /graphify
argument-hint: '[subfolder path to process, e.g. Notes/ or Journals/ — for very large
  vaults (1000+ files) subset instead of running on the full vault]'
...
- **Video/audio transcription** (`--whisper-model`): only relevant when Step 2's `detect` reports one or more `video` files. See [TRANSCRIBE.md]
