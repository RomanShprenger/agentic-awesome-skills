---
name: sunday-review
description: 'Use when the user asks for the weekly meta-review or Sunday wrap-up
  of their week and vault: says /sunday, /sunday-review, ''let''s do the weekly review,''
  ''Sunday review,'' ''review my week,'' ''weekly ret…'
...
Report a failure as a FAIL, never as a WARN, and carry it into Step 7 under "Vault state". Bug class `STAMP-GREEN-WHILE-ARTIFACT-GONE`: a check that probes the proxy (the file, the timestamp) instead of the leaf (the restorable content) stays green for exactly as long as the artifact is broken. Background: [`docs/BACKUP.md`].
