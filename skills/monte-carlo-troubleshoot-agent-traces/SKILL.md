...
description: Troubleshoots Monte Carlo AI agent alerts and traces — eval score drops,
  latency/token spikes, trajectory and validation breaches. Not for data incidents
  (monte-carlo-analyze-root-cause) or monitor c…
when_to_use: 'Use when the user wants to investigate an AI agent alert, trace, or
  behavior problem:
